...
class AiTextService
  ...
  def self.generate(tab:, source_text:, user_prompt:)
    new(tab:, source_text:, user_prompt:).generate
  end

  def initialize(tab:, source_text:, user_prompt:)
    @tab = tab
